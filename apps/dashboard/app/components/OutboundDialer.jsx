...
export function OutboundDialer({ tenantId }) {
  ...
  async function handleDial(e) {
    ...
    try {
      const res = await fetch("/api/outbound", {
        ...
        body: JSON.stringify({ tenant_id: tenantId, to: phone }),
      });
  ...
  return (
    <section className="bg-surface border border-line rounded-xl p-6">
      ...
      <h2 className="text-lg font-semibold text-ink tracking-tight mb-1">Ring ett nummer</h2>
      <p className="text-sm text-muted mb-5">
        Ulrika ringer direkt till numret du anger. Numret du ringer från är tenantens primära nummer.
      </p>

      <form onSubmit={handleDial} className="flex items-stretch gap-3">
        <input
          type="tel"
          value={phone}
