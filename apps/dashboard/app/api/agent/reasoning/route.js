...
import { patchReasoning } from "@/lib/control-plane";
...
export async function POST(req) {
  ...
  const { tenant_id, reasoning_effort } = body || {};
  if (!tenant_id) return Response.json({ error: "tenant_id required" }, { status: 400 });
  if (!reasoning_effort) return Response.json({ error: "reasoning_effort required" }, { status: 400 });

  try {
    const result = await patchReasoning(tenant_id, reasoning_effort);
