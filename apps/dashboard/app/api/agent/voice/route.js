import { getServerSession } from "next-auth";
import { authOptions } from "@/lib/auth-config";
import { effectiveScope } from "@/lib/tenant-map";
import { patchVoice } from "@/lib/control-plane";

export async function POST(req) {
  const session = await getServerSession(authOptions);
  if (!session?.user?.email) return Response.json({ error: "unauthenticated" }, { status: 401 });

  const { searchParams } = new URL(req.url);
  const scope = effectiveScope(session.user.email, Object.fromEntries(searchParams));
  if (!scope.admin && !scope.tenantId) return Response.json({ error: "no access" }, { status: 403 });

  let body;
  try { body = await req.json(); } catch { return Response.json({ error: "bad json" }, { status: 400 }); }

  const { tenant_id, voice } = body || {};
  const tenantId = scope.admin ? tenant_id : scope.tenantId;
  if (!tenantId) return Response.json({ error: "tenant_id required" }, { status: 400 });
  if (!scope.admin && tenantId !== scope.tenantId) return Response.json({ error: "not your tenant" }, { status: 403 });
  if (!voice) return Response.json({ error: "voice required" }, { status: 400 });

  try {
    const result = await patchVoice(tenantId, voice);
    return Response.json(result);
  } catch (err) {
    return Response.json({ error: err.message }, { status: err.status || 500 });
  }
}
